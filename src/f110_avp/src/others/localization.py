...
def convert_odom(v, omega, theta, time):
    result = np.zeros((4,))
    ...
    while True:
        ...
        if socket_result.poll(timeout = 5) != 0:
            ...
            if inference_in[1, 2] == 1:
                ...
                if last_time != 0:
                    ...
                    # filter jetter 
                    if delta_time < 0.2 and (delta_angular_speed > 2 or np.any(delta_linear_speed > 2)) and detection_count > calibration_count:
        ...
        if socket_odom.poll(timeout = 1) != 0 and detection_count > calibration_count:
            ...
            if delta_time_odom > 0.01 and z_now[2] != 0:
                ...
                odom_now = convert_odom(odom_info[0] * linear_speed_correction, odom_info[1] * angular_speed_correction, z_now[2], odom_info[2])
